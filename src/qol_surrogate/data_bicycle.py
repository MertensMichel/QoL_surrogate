"""[BICYCLE] Data aggregation, TAZ graph construction, Data-object building, and normalization.
...
HEX_PARQUET_DIR = "/mnt/raid1/MAAT/20.surrogate_data/cph/accessibility_bicycle/parquet"
TAZ_PARQUET_DIR = "/mnt/raid1/MAAT/20.surrogate_data/cph/accessibility_bicycle/parquet_taz"
DRY_BASELINE_DIR = "/mnt/raid1/MAAT/20.surrogate_data/cph/accessibility_bicycle/parquet_taz_baseline"
BASELINE_FILE = "/mnt/raid1/MAAT/20.surrogate_data/qol_surrogate/Copenhagen_acc_raindist_samples_BASELINE_BICYCLE.pkl"
...
# Copenhagen_9 is an exact, row-for-row duplicate of Copenhagen_4 (confirmed via full
# water_depths_BICYCLE content hash, same order) - keep Copenhagen_4, drop Copenhagen_9.
DUPLICATE_FILES = ["Copenhagen_9.parquet"]
...
def aggregate_to_taz(hexes, taz_ids, exclude_duplicates=True,
    ...
    Water depths: mean + p25/p50/p75/p90 per TAZ, over the BICYCLE edges that lie
    ...
    list_cols = ["water_depths_BICYCLE", "water_depths_BICYCLE_p25", "water_depths_BICYCLE_p50",
                 "water_depths_BICYCLE_p75", "water_depths_BICYCLE_p90"] + acc_cols
...
def save_pseudo_dry_baseline(taz_acc, output_dir=DRY_BASELINE_DIR):
    """Save the scenario with the lowest total BICYCLE water depth as a stand-in dry
    ...
    dry-run BICYCLE scenario exists (Copenhagen_acc_raindist_samples_BASELINE_BICYCLE.pkl).
    ...
    min_row = float('inf')
    min_id = -1
    for id, row in enumerate(taz_acc['water_depths_BICYCLE']):
        row_sum = sum(row)
        if row_sum < min_row:
            min_row = row_sum
            min_id = id
...
def save_dry_baseline(hexes, taz_ids, baseline_file=BASELINE_FILE, output_dir=DRY_BASELINE_DIR,
    ...
    (BASELINE_FILE) now points at a real dry-run BICYCLE scenario.
    ...
    acc_cols = [f'cumulative_accessibility_BICYCLE_{category}' for category in POI_CATEGORIES]
    needed_cols = ['water_depths_BICYCLE'] + acc_cols
...
def load_edge_taz_ids(taz_to_idx, network_dir=NETWORK_DIR):
    """Per-BICYCLE-edge TAZ assignment, in *positional* index space (0..len(taz_ids)-1,
    ...
    Some BICYCLE edges belong to a TAZ with no hexes (e.g. 103143 - has road edges but
    ...
    """
    edges = pickle.load(open(f"{network_dir}/BICYCLE_edges_Copenhagen.pkl", "rb"))
...
def load_dataset_tensors(static_features, taz_parquet_dir=TAZ_PARQUET_DIR, dry_baseline_dir=DRY_BASELINE_DIR):
    ...
    dynamic_cols = ["water_depths_BICYCLE", "water_depths_BICYCLE_p25", "water_depths_BICYCLE_p50",
                     "water_depths_BICYCLE_p75", "water_depths_BICYCLE_p90"]
    y_cols = [f"cumulative_accessibility_BICYCLE_{poi}" for poi in POI_CATEGORIES]
