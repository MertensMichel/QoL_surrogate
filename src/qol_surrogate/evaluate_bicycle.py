...
own buffer (model.dry_baseline) instead of a separate load - the bicycle
...
# Duplicated from qol_surrogate.data_bicycle.POI_CATEGORIES rather than imported -
...
def _predict_absolute(model, test_loader):
    ...
    evaluate_bicycle() and evaluate_per_taz_bicycle()."""
    model.eval()
    preds, targets = [], []
    with torch.no_grad():
...
def evaluate_bicycle(model, test_loader):
    """Evaluate the BICYCLE surrogate on a test set, in original (un-normalized)
...
def evaluate_per_taz_bicycle(model, test_loader, taz_ids):
    """{r2, mae, c_index, wape} (absolute accessibility) *per TAZ zone*, pooled
    across test scenarios and the 7 POI categories - the bicycle analogue of
    evaluate.evaluate_per_taz(), minus the per-mode split (bicycle is the only
    mode here).

    Returns {metric: DataFrame(taz_id -> value, column "BICYCLE")} - kept as a
    ...
    return {
        metric: pd.DataFrame({"BICYCLE": per_taz[metric]}, index=pd.Index(taz_ids, name="taz_id"))
        for metric in metric_names
    }
