...
# Copenhagen_9 is an exact, row-for-row duplicate of Copenhagen_5 (confirmed via full
# water_depths_CAR content hash, same order) - keep Copenhagen_5, drop Copenhagen_9.
...
def aggregate_to_taz(hexes, taz_ids, exclude_duplicates=True,
    ...
    Water depths: mean + p25/p50/p75/p90 per TAZ, over the CAR edges that lie
    ...
    list_cols = ["water_depths_CAR", "water_depths_CAR_p25", "water_depths_CAR_p50",
                 "water_depths_CAR_p75", "water_depths_CAR_p90"] + acc_cols
...
def save_dry_baseline(hexes, taz_ids, baseline_file=BASELINE_FILE, output_dir=DRY_BASELINE_DIR,
    ...
    acc_cols = [f'cumulative_accessibility_CAR_{category}' for category in POI_CATEGORIES]
    needed_cols = ['water_depths_CAR'] + acc_cols
...
def load_edge_taz_ids(taz_to_idx, network_dir=NETWORK_DIR):
    """Per-CAR-edge TAZ assignment, in *positional* index space (0..len(taz_ids)-1,
    ...
    Some CAR edges belong to a TAZ with no hexes (e.g. 103143 - has road edges but
    ...
    """
    edges = pickle.load(open(f"{network_dir}/CAR_edges_Copenhagen.pkl", "rb"))
...
def load_dataset_tensors(static_features, taz_parquet_dir=TAZ_PARQUET_DIR, dry_baseline_dir=DRY_BASELINE_DIR):
    ...
    dynamic_cols = ["water_depths_CAR", "water_depths_CAR_p25", "water_depths_CAR_p50",
                     "water_depths_CAR_p75", "water_depths_CAR_p90"]
    y_cols = [f"cumulative_accessibility_CAR_{poi}" for poi in POI_CATEGORIES]
