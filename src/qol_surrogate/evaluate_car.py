"""[CAR] TAZ-level evaluation metrics (R2, MAE, C-index, WAPE) for the CAR-only
...
own buffer (model.dry_baseline) instead of a separate load - the car
...
# Duplicated from qol_surrogate.data_car.POI_CATEGORIES rather than imported -
...
def _predict_absolute(model, test_loader):
    ...
    evaluate_car() and evaluate_per_taz_car()."""
    model.eval()
    preds, targets = [], []
    with torch.no_grad():
...
def evaluate_car(model, test_loader):
    """Evaluate the CAR surrogate on a test set, in original (un-normalized)
...
def evaluate_per_taz_car(model, test_loader, taz_ids):
    """{r2, mae, c_index, wape} (absolute accessibility) *per TAZ zone*, pooled
    across test scenarios and the 7 POI categories - the car analogue of
    evaluate.evaluate_per_taz(), minus the per-mode split (car is the only
    mode here).

    Returns {metric: DataFrame(taz_id -> value, column "CAR")} - kept as a
    ...
    return {
        metric: pd.DataFrame({"CAR": per_taz[metric]}, index=pd.Index(taz_ids, name="taz_id"))
        for metric in metric_names
    }
