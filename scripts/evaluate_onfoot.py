...
static-feature loading here - the onfoot checkpoint bundles all of that as
...
Usage:
    python scripts/evaluate_onfoot.py                    # evaluates the most recently trained onfoot run
    python scripts/evaluate_onfoot.py <path/to/run_dir>  # evaluates a specific run
"""
...
def build_test_loader(model):
    """Rebuild the same test split scripts/train_onfoot.py's build_data() produced
...
def main():
    ...
    taz_metrics = evaluate_onfoot(model, test_loader)
