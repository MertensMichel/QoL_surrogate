...
assumes scripts/agg_car.py has already been run to produce the cached dynamic parquet
and dry-baseline parquet. static_features.pkl/graph.pkl are NOT mode-specific (the
street-network/POI/geometry static features are identical regardless of transport
...
def build_data():
    ...
    sourced from scripts/agg_car.py's cached output.
    """
...
def save_run(model, train_losses, val_losses, idx_test, taz_ids, models_dir):
    ...
    run_dir = os.path.join(models_dir, f"gcn_resnet_car_{timestamp}")
...
# features in, 7 CAR POI-category accessibility-deviation outputs) - not tunable.
IN_CHANNELS = 29
OUT_CHANNELS = 7

# Same starting-point hyperparameters as scripts/train_onfoot.py - not yet tuned for
# this dataset specifically (4.5k CAR scenarios vs. onfoot's ~22k).
HIDDEN_CHANNELS = 256
N_LAYERS = 4
DROPOUT_RATE = 0.1
NUM_EPOCHS = 500
...
def main():
    ...
    # NOTE: no CAR-specific evaluate function exists yet (mirrors evaluate_onfoot.py's
    ...
    # needs an evaluate_car.py before this can report metrics beyond train/val loss curves.
