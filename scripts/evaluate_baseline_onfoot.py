...
notebooks/compare_baseline_onfoot.ipynb without recomputing.

Usage:
    python scripts/evaluate_baseline_onfoot.py                    # evaluates the most recently trained baseline run
    python scripts/evaluate_baseline_onfoot.py <path/to/run_dir>  # evaluates a specific run
"""
...
from qol_surrogate.baseline_onfoot import evaluate_baseline_onfoot, evaluate_per_taz_baseline_onfoot, flatten_x
...
def find_latest_run(models_dir=MODELS_DIR):
    run_dirs = sorted(d for d in glob.glob(os.path.join(models_dir, "gbm_onfoot_*")) if os.path.isdir(d))
    return run_dirs[-1]
...
def load_run(run_dir):
    ...
    return checkpoint["models"], checkpoint["idx_test"], checkpoint["taz_ids"]


def build_test_set(idx_test):
    # rebuild the same TAZ ordering and x/y tensors build_data() in
    # scripts/train_baseline_onfoot.py used - split_dataset() is deterministic
...
def main():
    ...
    models, idx_test, taz_ids = load_run(run_dir)
    ...
    taz_metrics = evaluate_baseline_onfoot(models, x_test_flat, y_test, dry_baseline)

    print("Computing per-TAZ metrics...")
    per_taz_metrics = evaluate_per_taz_baseline_onfoot(models, x_test_flat, y_test, taz_ids, dry_baseline)
