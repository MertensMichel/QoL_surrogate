...
from qol_surrogate.data import (
    ZONES_FILE, TAZ_PARQUET_DIR, DRY_BASELINE_DIR,
...
from qol_surrogate.evaluate import evaluate, evaluate_hex_level, evaluate_hex_ceiling
...
def build_test_loader(model, idx_test):
    ...
    return test_loader, hexes, taz_to_idx
...
def main():
    ...
    model, idx_test = load_model(run_dir)
    test_loader, hexes, taz_to_idx = build_test_loader(model, idx_test)

    print("Computing TAZ-level metrics...")
    taz_metrics = evaluate(model, test_loader)
    ...
    results = {"taz": taz_metrics, "hex": hex_metrics, "ceiling": ceiling_metrics}
