"""[BICYCLE] Entry point: computes TAZ-level evaluation metrics for a trained BICYCLE
surrogate run (model_architecture.GCNResNet, from scripts/train_bicycle.py) and saves
...
static-feature loading here - the bicycle checkpoint bundles all of that as
...
Usage:
    python scripts/evaluate_bicycle.py                    # evaluates the most recently trained bicycle run
    python scripts/evaluate_bicycle.py <path/to/run_dir>  # evaluates a specific run
"""
...
from qol_surrogate.evaluate_bicycle import evaluate_bicycle, evaluate_per_taz_bicycle
...
def find_latest_run(models_dir=MODELS_DIR):
    run_dirs = sorted(d for d in glob.glob(os.path.join(models_dir, "gcn_resnet_bicycle_*")) if os.path.isdir(d))
    return run_dirs[-1]


def build_test_loader(model):
    """Rebuild the same test split scripts/train_bicycle.py's build_data() produced
...
def main():
    ...
    taz_metrics = evaluate_bicycle(model, test_loader)

    print("Computing per-TAZ metrics...")
    per_taz_metrics = evaluate_per_taz_bicycle(model, test_loader, taz_ids)
